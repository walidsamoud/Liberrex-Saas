...
function getServicesByDepartment(department_id) {
    const requestOptions = {
        method: 'GET',
        headers: authHeader()
    };
    return fetch(`${ApiConfigs.base_url+ApiConfigs.private_services.getByDep.replace(':id', department_id)}`, requestOptions).then(handleResponse);
}
